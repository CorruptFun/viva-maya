...
| **Dashboard** (admin-gated read path) | `supabase/migrations/0013_analytics_dashboard.sql` + `stats.html` + `src/stats/` |
...
How it reads a table with **no SELECT policy**: it does not. `0013` adds `admin_analytics(p_days)`,
...
### 1b. Turn on the dashboard (`0013` — additive, any order vs the client deploy)

1. Paste `0013_analytics_dashboard.sql` into the SQL editor (same caveat: **not `sb push`** while
