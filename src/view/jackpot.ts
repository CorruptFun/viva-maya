...
/**
 * Dished-metal jackpot-wheel bezel drawn into `g`, centred on (cx,cy) radius R: a lathe-turned dish
 * (concentric bands graded bright outer lip → gold → deep → darkest groove toward the hub) under a fine
 * milled knurl whose teeth are lit BY ANGLE (bright at the crown, dark at the base) so the ring carries
 * both the radial dish AND a circumferential top-light in one pass, plus a warm crown light-pool. The
 * knurl stops at R+11, well inside the rim bulbs at R+20. Theme-token drawn (restyles per theme).
...
export function drawWheelBezel(
  ...
  const TAU = Math.PI * 2
  // Deep belly base — the wide ring the dish is turned into.
  g.lineStyle(20, T.goldDeep, 1)
  g.strokeCircle(cx, cy, R)
  // Dished cross-section: lit outer lip → gold upper wall → shaded lower wall → dark dish-bottom groove.
  g.lineStyle(3, T.goldBright, 0.9)
  g.strokeCircle(cx, cy, R + 6)
  g.lineStyle(6, T.gold, 1)
  g.strokeCircle(cx, cy, R + 2)
  g.lineStyle(8, T.goldDeep, 1)
  g.strokeCircle(cx, cy, R - 3)
  g.lineStyle(4, T.goldDarkest, 1)
  g.strokeCircle(cx, cy, R - 9)
  // Fine milled knurl on the outer edge (R+3 → R+11). Tooth tone by ANGLE: lit at the crown, dark at
  // the base; alternating length + weight = a milled shimmer chunky enough to survive the downscale.
  const teeth = 48
  for (let i = 0; i < teeth; i++) {
    const a = (i / teeth) * TAU
    const ca = Math.cos(a)
    const sa = Math.sin(a)
    const top = -sa // +1 at screen-top, −1 at the bottom
    const c = top > 0.45 ? T.goldBright : top > -0.15 ? T.gold : top > -0.6 ? T.goldDeep : T.goldDarkest
    const outer = i % 2 === 0 ? R + 11 : R + 8
    g.lineStyle(i % 2 === 0 ? 2.5 : 1.5, c, i % 2 === 0 ? 0.95 : 0.7)
    g.lineBetween(cx + ca * (R + 3), cy + sa * (R + 3), cx + ca * outer, cy + sa * outer)
  }
  // Warm crown light-pool + a tiny specular at the top of the band (the lathe sheen).
  g.fillStyle(T.glossHi, 0.1)
  g.fillEllipse(cx, cy - R, 54, 14)
 ...
 * (−26,−20)(26,−20)(0,30) — so the ±9° flex/spring the spin drives stays unchanged: a goldDeep cast
 * silhouette, inset gold flanks leaving a dark edge, a bright raised centre spine + specular sliver,
 * capped at the base by an offset-disc domed pivot rivet. Exported for the dev atlas ('wheel' page).
 */
export function drawWheelPointer(g: Phaser.GameObjects.Graphics, T: Theme = getTheme()): void {
  // Seat drop shadow (offset +3 down, matching the old pointer).
  g.fillStyle(T.shadow, 0.3)
  g.fillTriangle(-26, -17, 26, -17, 0, 33)
  // Cast silhouette (footprint corners preserved exactly) → inset gold flanks leaving a dark edge.
  g.fillStyle(T.goldDeep, 1)
  g.fillTriangle(-26, -20, 26, -20, 0, 30)
  g.fillStyle(T.gold, 1)
  g.fillTriangle(-20, -16, 20, -16, 0, 24)
  // Raised centre spine + a specular sliver along its crest.
  g.fillStyle(T.goldBright, 1)
  g.fillTriangle(-7, -16, 7, -16, 0, 26)
  g.fillStyle(T.glossHi, 0.5)
  g.fillTriangle(-2.5, -14, 2.5, -14, 0, 22)
  // Domed pivot rivet at the base (offset-disc idiom: deep base → lit dome offset up → crown → specular).
  g.fillStyle(0x000000, 0.18)
  g.fillEllipse(0, -6, 20, 7)
  g.fillStyle(T.goldDeep, 1)
  g.fillCircle(0, -11, 8)
  g.lineStyle(1.5, T.goldDarkest, 0.5)
  g.strokeCircle(0, -11, 8)
  g.lineStyle(1, T.goldBright, 0.5)
  g.strokeCircle(0, -11, 7)
  g.fillStyle(T.gold, 1)
  g.fillCircle(0, -12.5, 6)
  g.fillStyle(T.goldBright, 1)
  g.fillCircle(0, -13.5, 3.5)
  g.fillStyle(T.rim, 0.6)
  g.fillCircle(0, -14, 2)
  g.fillStyle(0xffffff, 0.9)
  g.fillCircle(0.5, -14.5, 1.2)
}
