...
export function makeMedal(scene: Phaser.Scene, rank: number, r: number): Phaser.GameObjects.Container {
  ...
  const g1 = rank === 1
  const faceCol = g1 ? T.gold : T.cardFillWarm
  const rimCol = g1 ? T.goldDeep : T.goldBezel
  // Deep ring under the face (offset down = the shaded underside) → the coin face.
  g.fillStyle(T.goldDarkest, g1 ? 0.5 : 0.34)
  g.fillCircle(0, 2.5, r)
  g.fillStyle(faceCol, 1)
  g.fillCircle(0, 0, r)
  // Milled/reeded rim — 18 alternating lit/shadowed radial teeth (chunky enough to survive r=26).
  const teeth = 18
  const inR = r * 0.8
  const outR = r * 0.97
  const tw = Math.max(1.6, r * 0.085)
  for (let i = 0; i < teeth; i++) {
    const a = (i / teeth) * Math.PI * 2 - Math.PI / 2
    const lit = i % 2 === 0
    g.lineStyle(tw, lit ? T.goldBright : T.goldDarkest, lit ? 0.85 : 0.7)
    g.lineBetween(Math.cos(a) * inR, Math.sin(a) * inR, Math.cos(a) * outR, Math.sin(a) * outR)
  }
  // Bevel rings (dark groove + lit ring) + the crisp outer edge → minted, not printed.
  g.lineStyle(Math.max(1.5, r * 0.06), T.goldDarkest, g1 ? 0.5 : 0.35)
  g.strokeCircle(0, 0, r * 0.9)
  g.lineStyle(Math.max(1.5, r * 0.05), T.goldBright, 0.4)
  g.strokeCircle(0, 0, r * 0.955)
  g.lineStyle(2.5, rimCol, 1)
  g.strokeCircle(0, 0, r)
  // Belly falloff (the lower half sinks into shadow) + a unifying gloss sweep across the upper half.
  g.fillStyle(0x000000, g1 ? 0.16 : 0.12)
  g.fillEllipse(0, r * 0.34, r * 1.5, r * 0.95)
  g.fillStyle(T.glossHi, g1 ? 0.2 : 0.26)
  g.fillEllipse(0, -r * 0.34, r * 1.4, r * 0.72)
  // Recessed engraved numeral field: a raised lit lip, a darker pressed disc, an inner-shadow crescent
  // at its top edge, and a faint light bounce at its base — so the numeral reads struck into the coin.
  g.lineStyle(Math.max(1.5, r * 0.05), T.goldBright, g1 ? 0.5 : 0.55)
  g.strokeCircle(0, 0, r * 0.6)
  g.fillStyle(T.goldDeep, g1 ? 0.28 : 0.18)
  g.fillCircle(0, r * 0.02, r * 0.56)
  g.fillStyle(0x000000, 0.14)
  g.fillEllipse(0, -r * 0.24, r * 0.92, r * 0.42)
  g.fillStyle(T.glossHi, 0.1)
  g.fillEllipse(0, r * 0.3, r * 0.8, r * 0.34)
  // Specular pip at the top-left crown.
  g.fillStyle(T.glossHi, 0.7)
  g.fillCircle(-r * 0.3, -r * 0.42, r * 0.1)
  c.add(g)
  const num = scene.add
