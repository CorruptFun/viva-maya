...
import { weekKey as senderWeekKey } from '../../scripts/send-push.mjs'
import { EVENTS, _queueDepth, _reset, analyticsEnabled, setAnalyticsEnabled, track } from './analytics'
import { weekKey as appWeekKey } from './endless'

/**
 * THE LOAD-BEARING TEST IN THIS FILE.
 *
 * scripts/send-push.mjs cannot import from src/ — it runs in CI as plain Node against the built
 * repo, with no TypeScript step — so it carries its own copy of weekKey(). A copy is a thing that
 * drifts, and this particular drift is SILENT AND TOTAL: the week key selects the leaderboard
 * partition, so a sender computing a different key reads an empty board and sends every player the
