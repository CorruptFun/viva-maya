...
export interface SaveData {
  ...
  /**
   * Latch for the one-time ⚡ LIGHTNING ROUND UNLOCKED reveal (`view/lightningunlockcard.ts`).
   * `seenRaceUnlock`'s twin: latched on RENDER, unioned on merge, absent in older saves → false.
   */
  seenLightningUnlock: boolean
  /**
   * Best ROUNDS SURVIVED in a lightning round, all-time.
   *
...
export function coerceSave(raw: unknown): SaveData {
  ...
  base.best = typeof data.best === 'number' ? data.best : 0
    ...
    // Absent in every save written before the storm shipped → unseen and no record, which is exactly
    // right: an existing player past level 5 gets the reveal on their next visit.
    base.seenLightningUnlock = data.seenLightningUnlock === true
    base.lightningBest =
...
/** Latch the ⚡ LIGHTNING ROUND reveal — `markRaceUnlockSeen`'s twin, latched on render. */
export function markLightningUnlockSeen(): void {
  const save = loadSave()
  if (!save.seenLightningUnlock) {
    save.seenLightningUnlock = true
    persistSave(save)
  }
}

/**
 * Record a finished lightning run. Returns the resulting best, and whether this run set it.
 *
