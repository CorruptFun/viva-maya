...
export interface SaveData {
  ...
  /** The invite code this player arrived through — a UI mirror of the 'viva-ton:ref' stash
...
export function coerceSave(raw: unknown): SaveData {
  ...
  base.best = typeof data.best === 'number' ? data.best : 0
    ...
    if (base.potTarget === 0) {
      base.potTarget = 500 + Math.floor(Math.random() * 401) // 500 to 900
    }
 ...
 * 'viva-ton:ref' localStorage stash (core/referrals.ts) stays authoritative for registration.
 */
