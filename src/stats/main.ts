 ...
 *
 * All reading goes through ONE RPC, admin_analytics (0013): the events table itself has no SELECT
...
function renderSignedOut(): void {
  ...
  page(
    ...
    notice(
      'Owner dashboard',
      'Gameplay analytics for Viva Maya. Sign in with an admin Google account to view — the data is unreadable without one (admin_analytics, 0013).',
      btn
    )
  )
}
...
function hourlyCard(a: Analytics): HTMLElement {
  ...
  return chartCard(
    ...
    'When the game actually gets played — the push reminders currently go out at 18:00 UTC.',
    chart,
...
function renderDashboard(session: Session, a: Analytics): void {
  ...
  foot.textContent = `Window: last ${a.meta.days} days, UTC · events are pruned after ~90 days, so “new device” means first seen within retention · aggregates only — no per-player rows leave the database (0013).`
...
async function load(session: Session): Promise<void> {
  ...
  if (error) {
    // 42501 is the deliberate "not an admin" answer from 0013 — a different page, not an error.
